...
export default address => dispatch => {
    ...
    return axios
        .get(`${GOOGLE_MAPS_GEOCODE_URL}/${address}&key=${GOOGLE_MAPS_GEOCODE_API_KEY}`)
        .then(res => dispatch(fetchAllApprovedCompaniesGeocodeSuccess(res.data)))
        .catch(err => dispatch(fetchAllApprovedCompaniesGeocodeFailure(err.message)));
};
