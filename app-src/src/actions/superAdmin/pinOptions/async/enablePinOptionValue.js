...
export const enablePinOptionValueRequest = payload => ({
    type: ENABLE_ADMIN_PIN_OPTION_VALUE_REQUEST,
    payload,
});

export const enablePinOptionValueSuccess = payload => ({
    type: ENABLE_ADMIN_PIN_OPTION_VALUE_SUCCESS,
    payload,
});

export const enablePinOptionValueFailure = (error, payload) => ({
    type: ENABLE_ADMIN_PIN_OPTION_VALUE_FAILURE,
    error,
    payload,
});
...
export default option => async dispatch => {
    ...
    return axios
        .patch(
            `${ADMIN_API_URL}/pinoptions/options/${option.id}/disable?undo=true`,
            null,
            getHeaders(),
        )
