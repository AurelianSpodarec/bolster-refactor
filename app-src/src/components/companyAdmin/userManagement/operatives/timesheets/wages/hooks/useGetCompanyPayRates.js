import { useEffect } from 'react';
import { useDispatch, useSelector } from 'react-redux';

import fetchPayRates from 'actions/companyAdmin/payRates/fetchPayRates';

import {
    selectPayRates,
    selectPayRatesError,
    selectPayRatesIsFetching,
} from 'selectors/companyAdmin/payRates';

const useGetCompanyPayRates = () => {
    const dispatch = useDispatch();

    const companyPayRates = useSelector(selectPayRates);
    const isFetching = useSelector(selectPayRatesIsFetching);
    const error = useSelector(selectPayRatesError);

    useEffect(() => {
        dispatch(fetchPayRates());
    }, []);

    return { companyPayRates, isFetching, error };
};

export default useGetCompanyPayRates;
