...
class EditCompanyPermissionsOnFloorFormContainer extends Component {
    state = {
        serviceIDs: [],
        services: []
    };
    ...
    render() {
        ...
        const backUrl = `/company/floors/${id}`;
        return (
    ...
    componentDidMount() {
        ...
        const { id } = match.params;
        fetchCompaniesPermissions('floor', id).then(() => {
            fetchAllServices();
        });
    ...
    componentDidUpdate(prevProps) {
        ...
        if (postSuccess && !prevProps.postSuccess)
            history.push(`/company/floors/${match.params.id}`);
    }
...
export default withRouter(
    connect(
        mapStateToProps,
        mapDispatchToProps
    )(EditCompanyPermissionsOnFloorFormContainer)
);
