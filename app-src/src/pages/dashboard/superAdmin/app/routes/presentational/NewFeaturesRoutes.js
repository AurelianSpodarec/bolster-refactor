import React from 'react';
import { Route } from 'react-router-dom';

import SwitchWith404 from 'components_DEPRECATED/appRoute/routes/presentational/SwitchWith404';
import NewFeaturesContainer from 'pages/dashboard/superAdmin/newFeatures/containers/NewFeaturesContainer';
import featureSingleContainer from 'pages/dashboard/superAdmin/newFeatures/containers/FeatureSingleContainer';

const NewFeaturesRoutes = ({ base = '/admin/new-features' }) => (
    <SwitchWith404>
        <Route exact path={base} component={NewFeaturesContainer} />
        <Route path={`${base}/:id`} component={featureSingleContainer} />
    </SwitchWith404>
);

export default NewFeaturesRoutes;
