...
const DashboardPinHistoryCharts = ({ isFetching, error, datasets, showLineGraph }) => {
    ...
    const updateDashboardFilters = useSelector(selectUpdateDashboardFilters);
    const prevProp = usePrevious(updateDashboardFilters.status);

    useEffect(() => {
        if (updateDashboardFilters.status !== prevProp) {
            setSelectedTab('pie');
        }
    }, [updateDashboardFilters, prevProp]);
